...
def test_fps_file_exists(host):
    ...
    fps_file = host.file("/var/run/media-bridge/fps_current")
    assert fps_file.exists, "FPS metrics file not found"


def test_fps_value_is_numeric(host):
    """Test that FPS value is a valid number."""
    fps_content = host.file("/var/run/media-bridge/fps_current").content_string.strip()
    try:
...
def test_dropped_frames_percentage_low(host):
    ...
    captured = int(host.file("/var/run/media-bridge/frames_total").content_string.strip())
    dropped = int(host.file("/var/run/media-bridge/frames_dropped").content_string.strip())
