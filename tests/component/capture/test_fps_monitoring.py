...
def test_fps_file_exists(host):
    ...
    fps_file = host.file("/var/run/ndi-bridge/fps_current")
    assert fps_file.exists, "FPS metrics file not found"


def test_fps_value_is_numeric(host):
    """Test that FPS value is a valid number."""
    fps_content = host.file("/var/run/ndi-bridge/fps_current").content_string.strip()
    try:
...
def test_fps_within_acceptable_range(host):
    """Test that FPS is within acceptable range (29-61 fps for 30/60fps targets)."""
    fps = float(host.file("/var/run/ndi-bridge/fps_current").content_string.strip())
    # Accept both 30fps and 60fps ranges
    assert (29.0 <= fps <= 31.0) or (59.0 <= fps <= 61.0), f"FPS {fps} outside acceptable range"


def test_frames_captured_increasing(host):
    """Test that frame counter is increasing over time."""
    # First reading
    frames1 = int(host.file("/var/run/ndi-bridge/frames_total").content_string.strip())
    
    # Wait and read again
    time.sleep(2)
    frames2 = int(host.file("/var/run/ndi-bridge/frames_total").content_string.strip())
...
def test_dropped_frames_percentage_low(host):
    ...
    captured = int(host.file("/var/run/ndi-bridge/frames_total").content_string.strip())
