...
const __m256i V4L2FormatConverterAVX2::Y_COEFF = _mm256_set1_epi16(298);
const __m256i V4L2FormatConverterAVX2::U_BLUE_COEFF = _mm256_set1_epi16(516);
const __m256i V4L2FormatConverterAVX2::U_GREEN_COEFF = _mm256_set1_epi16(-100);
...
bool V4L2FormatConverterAVX2::convertNV12toBGRA_AVX2(const uint8_t* input, int width, int height,
    ...
    // Process 16 pixels at a time for Y plane
    const int pixels_per_iteration = 16;
    const int aligned_width = (width / pixels_per_iteration) * pixels_per_iteration;
    ...
    for (int y = 0; y < height; y += 2) {
        ...
        for (int x = 0; x < aligned_width; x += pixels_per_iteration) {
            // Load Y values for both rows
            __m128i y0_128 = _mm_loadu_si128((const __m128i*)(y_row0 + x));
            __m128i y1_128 = _mm_loadu_si128((const __m128i*)(y_row1 + x));
            __m256i y0 = _mm256_cvtepu8_epi16(y0_128);
            __m256i y1 = _mm256_cvtepu8_epi16(y1_128);
            
            // Load and expand UV values
            __m128i uv_128 = _mm_loadu_si128((const __m128i*)(uv_row + x));
            
            // Separate U and V
            __m128i u_128 = _mm_and_si128(uv_128, _mm_set1_epi16(0x00FF));
            __m128i v_128 = _mm_srli_epi16(uv_128, 8);
            
            // Duplicate each U/V for horizontal expansion
            __m256i u = _mm256_cvtepu8_epi16(_mm_unpacklo_epi8(u_128, u_128));
            __m256i v = _mm256_cvtepu8_epi16(_mm_unpacklo_epi8(v_128, v_128));
            
            // Process both rows
            processYUV16_AVX2(_mm256_castsi256_si128(y0), _mm256_castsi256_si128(u), 
                              _mm256_castsi256_si128(v), dst_row0 + x * 4);
            processYUV16_AVX2(_mm256_castsi256_si128(y1), _mm256_castsi256_si128(u), 
                              _mm256_castsi256_si128(v), dst_row1 + x * 4);
        }
        
        // Handle remaining pixels
        for (int x = aligned_width; x < width; x++) {
            uint8_t y0_val = y_row0[x];
            uint8_t y1_val = y_row1[x];
